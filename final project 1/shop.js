...
function updateCart(){
    ...
    Object.keys(cart).forEach(product => {
        ...
        let quantityAndTrashContainer = document.createElement('div');
        quantityAndTrashContainer.classList.add('flex', 'items-center', 'space-x-2');
        ...
        trashIcon.classList.add('bx', 'bx-trash', 'cursor-pointer', 'text-red-500');
        trashIcon.onclick = function () {
            delete cart[product];
            updateCart();
            localStorage.removeItem(`cart_${product}_price`);
            localStorage.removeItem(`cart_${product}_quantity`);
            localStorage.removeItem(`cart_${product}_image`);
        };

        quantityAndTrashContainer.appendChild(quantityInput);
        quantityAndTrashContainer.appendChild(trashIcon);

        li.appendChild(productImage);
        li.appendChild(productInfo);
        li.appendChild(quantityAndTrashContainer);
